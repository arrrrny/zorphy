...
- The issue's example also shows the generated output as `abstract class $Task`. In the current zorphy output shapes, a plain entity (`abstract class Task`) generates a single concrete `class Task`; the `$`/`$$`-prefixed raw classes generate the abstract/sealed base and concrete subtype classes. Decorators port to EVERY generated class shape (concrete entity class, sealed base class, concrete subtype class), which is verified end-to-end; the literal `$Task` header in the example is illustrative rather than normative.
